...
class Negocio extends Migration
{
    ...
    public function up()
    {
        Schema::create('neg_negocio', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('neg_mun_fk');
            $table->foreign('neg_mun_fk')->references('mun_id')->on('cat_municipio');
            $table->string('neg_nombre_completo')->unique();
            $table->string('neg_nombre_corto')->unique();
            $table->string('neg_descripcion')->nullable();
            $table->string('neg_ubicacion')->nullable();
            $table->string('neg_logo')->nullable();
            $table->boolean("neg_servicio_dominicio")->default(false);
